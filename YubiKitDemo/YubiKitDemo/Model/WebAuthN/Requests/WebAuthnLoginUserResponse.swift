...
final class WebAuthnLoginUserResponse: NSObject, WebAuthnResponseProtocol {
    
    private(set) var uuid: String
    private(set) var hasWebAuthnAuthenticators = false
    ...
    init?(response: Data) {
        do {
            ...
            guard let dataUuid = userDictionary["uuid"] as? String else { return nil }
            uuid = dataUuid
            
            if let authenticatorsArray = userDictionary["authenticators"] as? Array<String> {
                hasWebAuthnAuthenticators = authenticatorsArray.contains("webauthn")
            }
        } catch _ {
            return nil
        }
