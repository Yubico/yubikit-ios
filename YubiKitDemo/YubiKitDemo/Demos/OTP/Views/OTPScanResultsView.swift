...
protocol OTPScanResultsViewDelegate: NSObjectProtocol {
...
class OTPScanResultsView: UIView {
    
    weak var delegate: OTPScanResultsViewDelegate?
    
    // MARK: Outlets
    
    @IBOutlet var tokenLabel: UILabel!
    @IBOutlet var moreDetailsButton: UIButton!
    @IBOutlet var testItButton: UIButton!
    ...
    @IBAction func moreDetailsButtonPressed(_ sender: Any) {
        if let delegate = delegate {
            delegate.otpScanResultsViewDidPressMoreDetails(self)
        }
    }
    
    @IBAction func testItButtonPressed(_ sender: Any) {
        if let delegate = delegate {
            delegate.otpScanResultsViewDidPressTestIt(self)
        }
    }
    
    @IBAction func rescanButtonPressed(_ sender: Any) {
        if let delegate = delegate {
            delegate.otpScanResultsViewDidPressRescan(self)
        }
    }
}
