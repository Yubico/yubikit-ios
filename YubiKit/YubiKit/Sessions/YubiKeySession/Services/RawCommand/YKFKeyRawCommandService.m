...
- (void)executeCommand:(YKFAPDU *)apdu completion:(YKFKeyRawCommandServiceResponseBlock)completion {
    ...
    [self.connectionController execute:apdu.apduData
        ...
        NSData *clearedResponseData = [self dataAndStatusFromKeyResponse:response];
        completion(clearedResponseData, nil);
    }];
}
...
- (void)executeSyncCommand:(YKFAPDU *)apdu completion:(YKFKeyRawCommandServiceResponseBlock)completion {
    ...
    [self.connectionController execute:apdu.apduData
        ...
        NSData *clearedResponseData = [strongSelf dataAndStatusFromKeyResponse:response];
        completion(clearedResponseData, nil);
        dispatch_semaphore_signal(semaphore);
    }];
