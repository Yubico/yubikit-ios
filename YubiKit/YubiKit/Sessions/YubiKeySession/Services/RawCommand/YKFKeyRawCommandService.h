...
/*!
 ...
 @discussion
    ...
    must not create one. It has to use only the single shared instance from YKFKeySession and sync its
    usage with the session state.
 */
@interface YKFKeyRawCommandService: YKFKeyService<YKFKeyRawCommandServiceProtocol>

/*
 Not available: use only the instance from the YKFKeySession.
 */
