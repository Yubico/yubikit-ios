...
- (void)executeOATHRequestWithoutApplicationSelection:(YKFKeyOATHRequest *)request completion:(YKFKeyOATHServiceResultCompletionBlock)completion {
    ...
    weakBlock = block = ^(NSData *result, NSError *error) {
        ...
        if (shortStatusCode == YKFKeyAPDUErrorCodeMoreData) {
            ...
            [strongSelf.connectionController execute:sendRemainingDataAPDU.apduData completion:strongBlock];
            return;
        }
    ...
    [self.connectionController execute:request.apdu.apduData completion:block];
}
