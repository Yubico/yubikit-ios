...
typedef NS_ENUM(NSUInteger, YKFPIVPinPolicy) {
    ...
    YKFPIVPinPolicyMatchOnce = 0x4,
    YKFPIVPinPolicyMatchAlways = 0x5
};
...
/// PIV error domain.
extern NSString* _Nonnull const YKFPIVErrorDomain;
...
@class YKFPIVSessionFeatures, YKFPIVManagementKeyType, YKFPIVManagementKeyMetadata, YKFPIVSlotMetadata, YKFPIVBioMetadata;
...
/// @abstract Response block for [getBioMetadata:completion:] which provides the bio key metadata or an error.
/// @param metaData The management key metadata.
/// @param error An error object that indicates why the request failed, or nil if the request was successful.
typedef void (^YKFPIVSessionBioMetadataCompletionBlock)
    (YKFPIVBioMetadata* _Nullable metaData, NSError* _Nullable error);

typedef void (^YKFPIVSessionBioVerifyUvCompletionBlock)
...
/// @abstract Reads metadata specific to YubiKey Bio multi-protocol.
/// @param completion The completion handler that gets called once the YubiKey has finished processing the request.
///                   This handler is executed on a background queue.
- (void)getBioMetadataWithCompletion:(nonnull YKFPIVSessionBioMetadataCompletionBlock)completion;

/// @abstract Authenticate with YubiKey Bio multi-protocol capabilities.
///
/// @discussion Before calling this method, clients must verify that the authenticator is bio-capable and
...
/// @param completion Temporary pin if requestTemporaryPin is true, otherwise null.
- (void)verifyUvRequestTemporaryPin:(bool)requestTemporaryPin checkOnly:(bool)checkOnly completion:(nonnull YKFPIVSessionBioVerifyUvCompletionBlock)completion
    NS_SWIFT_NAME(verifyUv(requestTemporaryPin:checkOnly:completion:));

/// @abstract Authenticate YubiKey Bio multi-protocol with temporary PIN.
///
/// @discussion The PIN has to be generated by calling `verifyUvRequestTemporaryPin:checkOnly:completion` and is
///             valid only for operations during this session and depending on pin policy.
///             Before calling this method, clients must verify that the authenticator is bio-capable and
///             not blocked for bio matching.
///
/// - Parameter pin: Temporary pin.
- (void)verifyTemporaryPin:(NSData *)pin completion:(nonnull YKFPIVSessionGenericCompletionBlock)completion;
