...
@interface YKFNFCReaderSessionTests: YKFTestCase

@property (nonatomic) YKFNFCReaderSession *nfcReaderSession;
@property (nonatomic) FakeNFCNDEFReaderSession *fakeReaderSession;
@property (nonatomic) YKFOTPTokenParser *tokenParser;

@end

@implementation YKFNFCReaderSessionTests
...
- (void)setUp {
    ...
    self.nfcReaderSession = [[YKFNFCReaderSession alloc] initWithTokenParser:self.tokenParser session:self.fakeReaderSession];
    
    self.fakeReaderSession.delegate = (id<NFCNDEFReaderSessionDelegate>)self.nfcReaderSession;
}
...
- (void)test_WhenNFCSessionInvalidatesWithError_ErrorIsReceived {
    ...
    [self.nfcReaderSession requestOTPToken:^(id<YKFOTPTokenProtocol> token, NSError *error) {
        if (error) {
            errorReceived = YES;
        }
    }];
